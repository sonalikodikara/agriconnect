...
import { Head, Link, router } from "@inertiajs/react"; // Added router for logout
...
interface User {
    ...
    name: string;
    email: string;
    role: string; // Assuming roles like 'farmer', 'advisor', etc.
}
...
export default function Home({ user }: HomeProps) {
    ...
    const handleLogout = () => {
        router.post('/logout'); // Assuming logout route is POST
    };
    ...
    const services = [
        ...
        { key: "pesticides", img: "/images/pesticide.png", products: ["Insecticide", "Fungicide"], route: route('list.pesticides') },
    ...
    return (
        <div className="min-h-screen bg-green-100 text-gray-800">
            ...
            <nav className="flex justify-between items-center p-4 bg-green-700 text-white fixed top-0 left-0 w-full z-50 shadow-lg">
                ...
                {/* Right Side: Dashboard Button, Profile Icon, Language Buttons */}
                <div className="flex items-center space-x-4">
                    <Link href={getDashboardRoute()} className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 shadow-md">
                        {t('Dashboard')}
                    </Link>
            ...
            {/* Mobile Menu (Separate Mobile View, Includes Language Buttons and Profile) */}
            {isMobileMenuOpen && (
                <div className="md:hidden bg-green-700 text-white p-4 fixed top-16 left-0 w-full z-40 shadow-lg">
                    ...
                    <div className="mt-4 border-t border-white/30 pt-4">
                        <Link href={getDashboardRoute()} className="block py-2 bg-yellow-500 text-white rounded px-4 hover:bg-yellow-600">
                            {t('Dashboard')}
                        </Link>
                        <div className="mt-2">
                            ...
                            <button onClick={handleLogout} className="mt-2 text-sm hover:text-yellow-300">
                                {t('Logout')}
                            </button>
                        </div>
                        <div className="mt-4">
            ...
            {/* Hero Section */}
            <header id="home" className="relative w-full h-screen pt-16 md:pt-0">
                    ...
                    {["/images/farming1.jpg", "/images/farm2.jpg", "/images/farming2.png"].map((img, idx) => (
                        <div key={idx} className="relative w-full h-screen">
                            ...
                            <div className="absolute inset-0 bg-black/40 flex flex-col justify-center items-center text-center text-white px-4">
                                <h1 className="text-4xl md:text-7xl font-bold mb-4 drop-shadow-lg">{t('welcome', { name: user.name })}</h1>
                                <Link href={getDashboardRoute()} className="px-4 md:px-6 py-2 md:py-3 bg-green-600 text-white rounded-lg text-base md:text-lg hover:bg-green-700 shadow-md">
                                    {t('Go to Dashboard')}
                                </Link>
                            </div>
                        </div>
