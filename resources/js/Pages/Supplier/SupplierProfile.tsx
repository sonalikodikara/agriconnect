...
export default function SupplierProfile() {
    ...
    // Form States (Pre-populate with user data)
    const [businessName, setBusinessName] = useState('');
    const [contactPerson, setContactPerson] = useState(auth.user.name || '');
    ...
    const handleCancel = () => {
        setBusinessName('');
        setContactPerson(auth.user.name || '');
            ...
            <div className="max-w-6xl mx-auto p-4 md:p-6">
                <div className="bg-white shadow-lg rounded-lg p-6 md:p-8 space-y-6">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                        <div>
                            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">{t('Supplier Profile Form')}</h1>
                            <p className="text-gray-600">{t('Welcome, {{name}}', { name: auth.user.name })}</p>
                        </div>
                        <button
                            type="button"
                            onClick={goHome}
                            className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 flex items-center gap-2 mt-4 md:mt-0"
                        >
                            <FaHome /> {t('Home')}
                        </button>
                    </div>
